...
function applyTransition(el: HTMLElement, transition: number, props: string[]) {
  if (transition > 0) {
    el.style.transition = props.map(p => `${p} ${transition}s ease-out`).join(', ');
  }
}

export class ScaleStrategy implements AdaptStrategy {
  apply(el: HTMLElement, info: StrategyApplyInfo): void {
    const { scale, translateX, translateY, designW, designH, direction, overflow, transition } = info;
...
export class VwvhStrategy implements AdaptStrategy {
  apply(el: HTMLElement, info: StrategyApplyInfo): void {
    const { designW, designH, direction, overflow, transition, viewportW, viewportH } = info;
